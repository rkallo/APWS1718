...
x1,I1 = np.loadtxt("einzelspalt_s.txt", unpack=True, delimiter=',') # loadtxt liest Daten aus Tabellen-datei ein, unpack = True damit die Arrays richtig zugordnet werden
x2,I2 = np.loadtxt("einzelspalt_b.txt", unpack=True, delimiter=',')
x3,I3 = np.loadtxt("doppelspalt2.txt", unpack=True, delimiter=',')
...
var3, f_var3=opt.curve_fit(I_phi2, x3, I3, [1, 40e-6,25e-3, 0.25e-3], maxfev=10000)

x_werte=np.linspace(-0.04 ,0.04, 10000) # linspace(a,b, N) erstellt Array mit N Werten von a bis b 
...
plt.plot(1.3*x_werte,I_phi1((x_werte+var1[2]), var1[0], var1[1], var1[2])/(650e-9), 'r-', label=r"$\mathrm{Fit}$")
...
#plt.plot(1.3*x_werte,I_phi1((x_werte+var2[2]), var2[0], var2[1], var2[2])/(480e-9), 'r-', label=r"$\mathrm{Fit}$")
#plt.xlabel("Winkel in rad")
#plt.ylabel("Normierte Intesität")
#plt.legend()
#plt.grid()
plt.savefig('doppelspalt2.pdf')
#plt.show()

#plt.plot(phi3,I3/(100e-9),"b.",label="Messwerte")
#plt.plot(1.3*x_werte,I_phi2((x_werte+var3[2]), var3[0], var3[1], var3[2], var3[3])/(100e-9), 'r-', label=r"$\mathrm{Fit}$")
#plt.xlabel("Winkel in rad")
#plt.ylabel("Normierte Intesität")
#plt.legend()
#plt.grid()
